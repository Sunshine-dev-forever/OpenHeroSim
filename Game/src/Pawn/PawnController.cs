...
public partial class PawnController : Node, IPawnController {
	...
	public override void _Ready() {
		...
		PawnTaskHandler = new PawnTaskHandler(
			PawnMovement,
			PawnVisuals,
			PawnInformation,
			PawnInventory);
	...
	public override void _PhysicsProcess(double delta) {
		if (!IsDying) {
			...
			// _PhysicsProcess
			PawnTaskHandler.HandleTask(currentTask);
		}
	}
	...
	public double GetDamage() => PawnInformation.BaseDamage + PawnInventory.GetTotalEquiptmentDamage();
	...
	void CreateGravestone() {
		Node3D TreasureChestMesh = CustomResourceLoader
			.LoadMesh(ResourcePaths.GRAVESTONE);

		gravestone = new ItemContainer(
			PawnInventory.EmptyAllItems(),
			TreasureChestMesh);
		...
		gravestone.GlobalTransform = new Transform3D(gravestone.GlobalTransform.Basis, GlobalTransform.Origin);
	...
	IDisplay ConstructDisplay() {
		...
		childBaggedItems.AddDetail("number of bagged items: " +
			PawnInventory.GetAllItemsInBag().Count);
