...
public partial class InGameUI : Control {
    ...
    public override void _Input(InputEvent input) {
        if (input.IsActionPressed("mouse_left_click") && input is InputEventMouseButton button) {
            CastRayFromCamera(button);
        }
    }
    ...
    void CastRayFromCamera(InputEventMouseButton input) {
        ...
        if (result.Count == 0) {
            ...
            Visible = true;
            VBoxContainer vBoxContainer = GetNode<VBoxContainer>("VBoxContainer");
            SceneTreeUtil.RemoveAndFreeAllChildren(vBoxContainer);

            IInteractable target = interactables[0];

            if (target is IPawnController controller) {
                AddPawnInformation(controller);
            }
            else if (target is ItemContainer container) {
                AddItemContainerInformation(container);
            }
        }
    }
